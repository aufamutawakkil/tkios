...
export default class TokoList extends React.Component{
    constructor(props){
        super(props);
        this.state = {
            dataSource:[],
    ...
    componentWillMount(){
        this._renderToko(this.props.mainKatId);
    }

    _renderToko = (id) => {
        Req.start(URL.baseUrl + "toko/get_toko_by_main_kat/" + id + "?filter=" + "All" + "&sort=" + "Terbaru" ,"GET",false,false,(res)=>{
            var tokos = res.data.toko;     
            var dataSource = [];
            tokos.map((item)=> {
